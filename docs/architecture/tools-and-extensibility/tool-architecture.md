...
Choose a clear name and a descriptive `description` (this is what the LLM uses to decide when to call the tool). Define `parameters` as a JSON Schema object:
...
### 2. Design Output Channels

Decide whether the tool's output belongs in the chat history or the clipboard:
- **Chat history** (`tool_response`): short confirmations, errors, summaries
- **Clipboard** (`results_to_add_to_clipboard`): large text the agent should read without cluttering context
- **Files** (`files_to_add_to_clipboard`): files the agent should inspect on the next turn
- **Entities** (`entities_to_track`): productivity items the agent should monitor across turns

### 3. Implement the Run Method

```python
async def run(self, working_directory: Path, query: str,
...
In `myproject-tools/src/myproject_tools/registry.py`, add:
...
- **Async only** — tools must be `async`. Use `asyncio.to_thread()` for blocking I/O (file reads, HTTP calls).
- **Never block the event loop** — long operations must be offloaded to a thread.
- **Always validate paths** — call `_validate_path()` before any file operation, even reads.
- **Return `ToolResult`**, never raise. Tool execution errors should be caught and returned as `status="error"` with a descriptive `tool_response`.
